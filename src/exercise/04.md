...
Learnt a couple of ways to create components with JSX. Engage with JSX specifications a bit such as some kind of naming convention doesn't work with JSX specification.

PropTypes are a helpful material if not using Typescript at all. The main difference between TS and PropTypes is TS makes type check at the compilation time, PropTypes does it on RunTime so it has a performance effect so should not be used in production build.

I remember I was creating unneccesary elements just to surround my components before knowing Fragments. React delivers a solution to almost any ugly problem so I'd be aware at this more and I'll check through if there's a nice solutino rather than ignoring these bad practices.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=han.tuerker%40gmail.com).
